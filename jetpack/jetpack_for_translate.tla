...
CONSTANTS Server, Client, CmdId, Key, NoOpCmd,
          ...
          ProposerOf         \* [Server -> Proposer] (which proposer a server uses)
...
\* Commands, View, JPool, PrepResp type/set definitions unsupported by translate-tla.
\* Multi-variable set comprehension with record construction: { [f |-> x, ...] : x \in S, ... }
\* Record set type notation: [field: Type, ...]
\* All will be hand-translated as Verus structs.
\* REMOVED FOR TRANSLATOR PASS:
...
\* JPool == [max_seen_ballot: Nat, ...] — record set type, removed
\* EmptyJPool — record literal with function construction [k \in Key |-> NilCmd], removed
\* JPoolCommands(p) — set map {p.pool[k] : k \in Key} \ {NilCmd}, removed
\* PrepResp == [...] — record set type, removed
...
\* RemoveCmd and Dedup use \o (sequence concat) which is unsupported by translate-tla.
\* These recursive helpers will be hand-translated to Verus spec functions.
\* REMOVED FOR TRANSLATOR PASS:
...
\* FilterNoOps uses SelectSeq + LAMBDA, unsupported by translate-tla.
\* Will be hand-translated. REMOVED FOR TRANSLATOR PASS.
